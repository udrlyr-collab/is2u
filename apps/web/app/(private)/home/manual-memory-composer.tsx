...
function FilePicker({ type, file, onFile }: { type: "photo" | "video"; file: File | null; onFile: (file: File | null) => void }) {
...
function AudioPicker({ file, onFile, onNotice }: { file: File | null; onFile: (file: File | null) => void; onNotice: (notice: Notice) => void }) {
...
export function ManualMemoryComposer({ onSaved }: { onSaved: () => void }) {
  ...
  function reset() {
    setType(null); setTitle(""); setText(""); setFile(null); setProgress(null); setNotice(null);
  }
  ...
  return <section className="manual-memory-composer">
    ...
    {open && <div className="manual-memory-sheet">
      {!type && <><p className="paper-label">MEMORY PIECES</p><h2>어떤 추억을 남길까요</h2><div className="manual-memory-choices"><button type="button" disabled={busy} onClick={() => void makeRandomMission()}><span aria-hidden="true">✦</span>무작위 미션 받기</button>{choices.map((choice) => <button type="button" key={choice.type} onClick={() => { setType(choice.type); setNotice(null); }}><span aria-hidden="true">{choice.mark}</span>{choice.label}</button>)}</div></>}
