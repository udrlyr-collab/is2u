...
export const DELETE = withApiErrors(async (request: Request, context: Context) => {
  ...
  const now = new Date();
  const [memory] = await getDb().update(memories).set({ deletedAt: now, purgeAfter: new Date(now.getTime() + 30 * 24 * 60 * 60_000) }).where(and(eq(memories.id, id), isNotNull(memories.createdAt))).returning();
...
export const PATCH = withApiErrors(async (request: Request, context: Context) => {
  ...
  await requireCsrf(request, session);
  const { id } = await context.params;
  const [memory] = await getDb().update(memories).set({ deletedAt: null, purgeAfter: null }).where(eq(memories.id, id)).returning();
  if (!memory) throw new HttpError(404, "기억을 찾을 수 없습니다.");
