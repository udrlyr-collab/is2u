...
import { cancelScheduledMission, rescheduleMissionForDate } from "../../../../lib/scheduler";
import { canAccessCouple, requireActiveRecordCouple } from "../../../../lib/couples";
...
export const GET = withApiErrors(async (request: Request, context: Context) => {
  ...
  if (!await canAccessCouple(session.user.id, event.coupleId)) throw new HttpError(404, "약속을 찾을 수 없어요");
...
export const PATCH = withApiErrors(async (request: Request, context: Context) => {
  ...
  if (updated.status === "active" || updated.status === "scheduled") await rescheduleMissionForDate(id);
...
export const DELETE = withApiErrors(async (request: Request, context: Context) => {
  ...
  const [updated] = await getDb().update(dateEvents).set({ deletedAt: new Date(), updatedAt: new Date() }).where(and(eq(dateEvents.id, id), eq(dateEvents.coupleId, existing.coupleId), eq(dateEvents.isTest, false), isNull(dateEvents.deletedAt))).returning();
